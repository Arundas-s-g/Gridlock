...
for df in [train_49, exam_split]:
    ...
    # DIRECT MULTIPLICATION PRODUCT FEATURES
    df['demand_prev_day_x_sin'] = df['demand_prev_day'] * df['time_slot_sin']
    df['demand_prev_day_x_cos'] = df['demand_prev_day'] * df['time_slot_cos']
    df['demand_prev_day_x_slot'] = df['demand_prev_day'] * (df['time_slot'] / 96.0)
...
train_48['demand_prev_day_x_sin'] = np.nan
train_48['demand_prev_day_x_cos'] = np.nan
train_48['demand_prev_day_x_slot'] = np.nan
...
features = [
    ...
    'demand_prev_day_x_sin', 'demand_prev_day_x_cos', 'demand_prev_day_x_slot'
]
...
print(f"HOLD OUT EXAM R2: {r2:.5f} ({r2*100:.2f}%)")
