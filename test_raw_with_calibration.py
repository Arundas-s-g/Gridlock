...
# Calculate geohash-specific mean demand for slots 0-8
d48_start = train_48[train_48["time_slot"].isin([0, 1, 2, 3, 4, 5, 6, 7, 8])]
d49_start = train_49[train_49["time_slot"].isin([0, 1, 2, 3, 4, 5, 6, 7, 8])]
...
d48_road = d48_start.groupby("RoadType")["demand"].mean().to_dict()
d49_road = d49_start.groupby("RoadType")["demand"].mean().to_dict()

road_ratios = {}
for r in train_df["RoadType"].unique():
    m48 = d48_road.get(r, 0.0)
    m49 = d49_road.get(r, 0.0)
    if m48 > 0.01:
        road_ratios[r] = np.clip(m49 / m48, 0.5, 2.0)
    else:
        road_ratios[r] = global_ratio

# Populate Calibration Features
for df in (train_df, holdout_df):
    df["global_early_ratio"] = df.apply(lambda r: global_ratio if r["day"] == 49 else 1.0, axis=1)
    df["geo_early_ratio"] = df.apply(lambda r: location_ratios.get(r["geohash_raw"], global_ratio) if r["day"] == 49 else 1.0, axis=1)
    df["road_early_ratio"] = df.apply(lambda r: road_ratios.get(r["RoadType"], global_ratio) if r["day"] == 49 else 1.0, axis=1)

# Apply Day 48 Anchors (not scaled during training/features to let models learn scale natively!)
...
features = [
    ...
    "global_early_ratio", "geo_early_ratio", "road_early_ratio"
]

print("Training Ensemble models on Raw target with Calibration Features...")
models = [
...
# ==============================================================================
print("\nRunning Hierarchical Fallback Recursive Forecasting...")
...
for _, slot_frame in ordered_holdout.groupby("time_slot", sort=True):
    ...
    for lag in (1, 2, 3):
        # Look up recursive same-day, else look up Day 48 (anchoring is native, no manual scaling), else fallback
        l_preds = []
        for geohash in batch["geohash_raw"]:
            if (geohash, slot - lag) in d49_history:
                val = d49_history[(geohash, slot - lag)]
            else:
                val = d48_same_slot_dict.get((geohash, slot - lag), fallback)
            l_preds.append(val)
...
print("\n========================================================")
print(f"Hierarchical Fallback + Calibration Features R2 Score: {r2*100:.5f}% (R2: {r2:.5f})")
print("========================================================")
