...
print("=== Starting V2 Final Optimized ML Pipeline ===")
...
# 5. Collapse rare geohashes (frequency < 5 in training data)
...
# 6. Fill missing Temperature, Weather, and RoadType
...
# 8. Interaction Features (Pruning weak ones like geo_weather and road_weather)
...
# 9. Lags and Rolling Statistics on Day 48
print("Engineering lags & rolling statistics on Day 48 series...")
...
# Generate lags/rolling features on Day 48 for Day 49 train and test sets
for df in [train_49, test]:
    ...
    # Impute missing lags/demand lookups with slot mean from Day 48
...
# For Day 48 training rows, lags and demand_prev_day are NaN (no Day 47 reference)
...
# Add Statistical Group Aggregations on Day 48
...
for col in cat_cols:
    le = LabelEncoder()
    if col == 'geohash':
        combined = pd.concat([train_48['geohash_raw'].astype(str), train_49['geohash_raw'].astype(str), test['geohash_raw'].astype(str)])
        le.fit(combined)
        train_48['geohash_encoded'] = le.transform(train_48['geohash_raw'].astype(str))
        ...
        combined = pd.concat([train_48[col].astype(str), train_49[col].astype(str), test[col].astype(str)])
        le.fit(combined)
        train_48[col] = le.transform(train_48[col].astype(str))
        train_49[col] = le.transform(train_49[col].astype(str))
        test[col] = le.transform(test[col].astype(str))

# 12. Define Feature Space
features = [
...
# 13. Fold-Safe Target Encoding CV over Train day 49
...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_49)):
    ...
    # Fold-safe target encoding computed on combined_train_fold
    for col in te_cols:
    ...
    # Weighted predictions for test split (0.5 LGBM + 0.35 XGB + 0.15 CatBoost)
    test_preds += (0.5 * lgb.predict(X_te_fit) + 0.35 * xgb.predict(X_te_fit) + 0.15 * cat.predict(X_te_fit)) / 5.0

# Clip and save submission file
...
print("\n=== Saved V2 Leakage-Free Optimized Submission ===")
