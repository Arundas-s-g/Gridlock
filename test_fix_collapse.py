...
# Keep original raw geohash strings for lookups (NEVER collapse this for lookups!)
train_split['geohash_lookup'] = train_split['geohash'].astype(str)
exam_split['geohash_lookup'] = exam_split['geohash'].astype(str)
...
geohashes = pd.concat([train_split['geohash_lookup'], exam_split['geohash_lookup']]).unique()
geo_coords = {}
for g in geohashes:
    try:
...
train_split['lat'] = train_split['geohash_lookup'].map(lambda g: geo_coords[g][0])
train_split['lon'] = train_split['geohash_lookup'].map(lambda g: geo_coords[g][1])
exam_split['lat'] = exam_split['geohash_lookup'].map(lambda g: geo_coords[g][0])
exam_split['lon'] = exam_split['geohash_lookup'].map(lambda g: geo_coords[g][1])
...
geohash_counts = train_split['geohash_lookup'].value_counts()
rare_geohashes = set(geohash_counts[geohash_counts < 5].index)
train_split['geohash_cat'] = train_split['geohash_lookup'].apply(lambda g: 'OTHER' if g in rare_geohashes else g)
exam_split['geohash_cat'] = exam_split['geohash_lookup'].apply(lambda g: 'OTHER' if g in rare_geohashes else g)

# Impute temperature, weather, road type
temp_median = train_split['Temperature'].median()
geo_temp_median = train_split.groupby('geohash_lookup')['Temperature'].median().to_dict()

def impute_temp(row):
    if pd.isna(row['Temperature']):
        return geo_temp_median.get(row['geohash_lookup'], temp_median)
...
# Prefixes on the original geohash
train_split['geo_4'] = train_split['geohash_lookup'].str[:4]
train_split['geo_5'] = train_split['geohash_lookup'].str[:5]
exam_split['geo_4'] = exam_split['geohash_lookup'].str[:4]
exam_split['geo_5'] = exam_split['geohash_lookup'].str[:5]
...
def create_interactions(df):
    ...
    df['geo_hour'] = df['geohash_lookup'] + "_" + df['hour'].astype(str)
    return df
...
# Lookups are strictly on original raw geohashes
d48_demand_dict = train_48.set_index(['geohash_lookup', 'time_slot'])['demand'].to_dict()
...
for df in [train_49, exam_split]:
    df['demand_prev_day'] = df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 0), axis=1)
    df['lag_1_d48'] = df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], -1), axis=1)
    df['lag_2_d48'] = df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], -2), axis=1)
    df['lag_3_d48'] = df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], -3), axis=1)
    df['lag_6_d48'] = df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], -6), axis=1)
    df['lag_12_d48'] = df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], -12), axis=1)
    df['lag_24_d48'] = df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], -24), axis=1)
    ...
    df['rolling_mean_3_d48'] = (df['lag_1_d48'] + df['demand_prev_day'] + df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 1), axis=1).fillna(df['slot_mean_d48'])) / 3.0
    df['rolling_mean_6_d48'] = (df['lag_2_d48'] + df['lag_1_d48'] + df['demand_prev_day'] + 
                                df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 1), axis=1).fillna(df['slot_mean_d48']) + 
                                df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 2), axis=1).fillna(df['slot_mean_d48']) + 
                                df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 3), axis=1).fillna(df['slot_mean_d48'])) / 6.0
    df['rolling_mean_12_d48'] = (df['lag_6_d48'] + df['lag_3_d48'] + df['lag_2_d48'] + df['lag_1_d48'] + df['demand_prev_day'] + 
                                 df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 1), axis=1).fillna(df['slot_mean_d48']) + 
                                 df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 2), axis=1).fillna(df['slot_mean_d48']) + 
                                 df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 3), axis=1).fillna(df['slot_mean_d48']) +
                                 df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 4), axis=1).fillna(df['slot_mean_d48']) +
                                 df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 5), axis=1).fillna(df['slot_mean_d48']) +
                                 df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 6), axis=1).fillna(df['slot_mean_d48'])) / 11.0
                                 
    df['rolling_std_3_d48'] = np.std([df['lag_1_d48'], df['demand_prev_day'], df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 1), axis=1).fillna(df['slot_mean_d48'])], axis=0)
    df['rolling_max_3_d48'] = np.max([df['lag_1_d48'], df['demand_prev_day'], df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 1), axis=1).fillna(df['slot_mean_d48'])], axis=0)
    df['rolling_min_3_d48'] = np.min([df['lag_1_d48'], df['demand_prev_day'], df.apply(lambda r: get_d48_demand(r['geohash_lookup'], r['time_slot'], 1), axis=1).fillna(df['slot_mean_d48'])], axis=0)
...
geo_mean_d48 = train_48.groupby('geohash_lookup')['demand'].mean().to_dict()
geo_std_d48 = train_48.groupby('geohash_lookup')['demand'].std().to_dict()
...
for df in [train_48, train_49, exam_split]:
    df['geo_mean_d48'] = df['geohash_lookup'].map(geo_mean_d48).fillna(df['time_slot'].map(slot_mean_d48))
    df['geo_std_d48'] = df['geohash_lookup'].map(geo_std_d48).fillna(0.0)
...
freq_cols = ['geohash_lookup', 'geo_hour', 'road_hour']
...
features = [
    ...
    'geohash_lookup_freq', 'geo_hour_freq', 'road_hour_freq', 'is_weekend', 'is_peak_hour', 'is_night', 'is_office_hour', 'spatial_cluster'
]
...
print(f"FIXED COLLAPSE BUG - HOLD OUT EXAM R2: {r2:.5f} ({r2*100:.2f}%)")
