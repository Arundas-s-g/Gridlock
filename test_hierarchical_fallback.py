...
print("Training Mode 3 Ensemble models...")
models = run_training(train_features_df, features) if 'run_training' in globals() else None
# Define inline run_training to keep it self-contained
if models is None:
    models = [
...
# Map Day 48 same-slot dictionary for precise lookups
...
for _, slot_frame in ordered_holdout.groupby("time_slot", sort=True):
    ...
    for lag in (1, 2, 3):
        # HIERARCHICAL FALLBACK LOOKUP:
        # 1. Look up recursive same-day demand from Day 49 (d49_history)
        # 2. If missing, look up exact location same-slot demand from Day 48 (d48_same_slot_dict)
        # 3. If missing, fall back to global slot average on Day 48 (fallback)
    ...
    preds = predict_ensemble(models, batch[features]) if 'predict_ensemble' in globals() else np.clip(np.column_stack([m.predict(batch[features]) for m in models]) @ np.array([0.15, 0.45, 0.40]), 0.0, 1.0)
...
print("\n========================================================")
print(f"Hierarchical Fallback R2 Score: {r2*100:.5f}% (R2: {r2:.5f})")
print("========================================================")
