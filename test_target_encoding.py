...
def clean_and_prep(raw_train):
    ...
    for df in (train_df, holdout_df):
        ...
        # Golden Interaction Feature: geohash x time_slot
...
# Label Encode the interaction keys for Target Encoding
te_encoder = LabelEncoder()
combined_te = pd.concat([train_features_df["geo_time_slot"], holdout_df["geo_time_slot"]])
te_encoder.fit(combined_te)
train_features_df["geo_time_slot_encoded"] = te_encoder.transform(train_features_df["geo_time_slot"])
holdout_df["geo_time_slot_encoded"] = te_encoder.transform(holdout_df["geo_time_slot"])

# Target Encoding Feature Space
features = [
    ...
    "geo_time_slot_te" # Target encoded feature
]

print("Running 5-fold cross-validation with safe out-of-fold target encoding...")
kf = KFold(n_splits=5, shuffle=True, random_state=42)

# Prepare out-of-fold target encodings for train
train_features_df["geo_time_slot_te"] = np.nan
holdout_df["geo_time_slot_te"] = 0.0

global_mean = train_features_df["demand"].mean()

# Calculate OOF target encoding
for train_idx, val_idx in kf.split(train_features_df):
    tr_fold = train_features_df.iloc[train_idx]
    te_map = tr_fold.groupby("geo_time_slot_encoded")["demand"].mean().to_dict()
    
    # Fill validation fold
    train_features_df.iloc[val_idx, train_features_df.columns.get_loc("geo_time_slot_te")] = (
        train_features_df.iloc[val_idx]["geo_time_slot_encoded"].map(te_map).fillna(global_mean)
    )
    
    # Accumulate for holdout test set
    holdout_df["geo_time_slot_te"] += holdout_df["geo_time_slot_encoded"].map(te_map).fillna(global_mean) / 5.0

# Train the final models
print("Training Ensemble models...")
models = [
    LGBMRegressor(n_estimators=1200, learning_rate=0.025, num_leaves=31, random_state=42, verbose=-1),
    XGBRegressor(n_estimators=1200, learning_rate=0.025, max_depth=5, random_state=42, verbosity=0, n_jobs=1),
    CatBoostRegressor(iterations=1400, learning_rate=0.025, depth=5, random_state=42, allow_writing_files=False, verbose=0)
]
for model in models:
    model.fit(train_features_df[features], train_features_df["demand"])

# ==============================================================================
# HIERARCHICAL RECURSIVE FORECASTING
# ==============================================================================
print("\nRunning Hierarchical Fallback Recursive Forecasting with Target Encoding...")
...
for _, slot_frame in ordered_holdout.groupby("time_slot", sort=True):
    ...
    for lag in (1, 2, 3):
        batch[f"same_day_lag_{lag}"] = [
            d49_history.get((geohash, slot - lag), 
                            d48_same_slot_dict.get((geohash, slot - lag), 
                                                   fallback))
    ...
    preds = np.clip(predictions @ weights, 0.0, 1.0)
...
print("\n========================================================")
print(f"Hierarchical Fallback + Target Encoding R2 Score: {r2*100:.5f}% (R2: {r2:.5f})")
print("========================================================")
