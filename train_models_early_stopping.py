...
print("\nEvaluating models with early stopping on Day 49...")
for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
    X_tr, y_tr = X.iloc[train_idx], y.iloc[train_idx]
    X_va, y_va = X.iloc[val_idx], y.iloc[val_idx]
    
    # LightGBM
    lgb = LGBMRegressor(n_estimators=2000, learning_rate=0.03, random_state=42, verbose=-1)
    lgb.fit(
        X_tr, y_tr,
        eval_set=[(X_va, y_va)],
    ...
    # XGBoost
    xgb = XGBRegressor(n_estimators=2000, learning_rate=0.03, early_stopping_rounds=50, random_state=42, verbosity=0)
    xgb.fit(
        X_tr, y_tr,
        eval_set=[(X_va, y_va)],
        verbose=False
    ...
    # CatBoost
    cat = CatBoostRegressor(iterations=2000, learning_rate=0.03, early_stopping_rounds=50, random_state=42, verbose=0)
    cat.fit(
        X_tr, y_tr,
        eval_set=[(X_va, y_va)],
        verbose=False
